...
function teamPopup(parent) {
    ...
    function modalBuild(array) {
        ...
        const modalExcerpt = array.bio;
        const modalImageURL = array.image;
        const modalImageAlt = array.image_alt;

        let excerpt =
            modalExcerpt && typeof modalExcerpt !== 'undefined'
                ? `<div class="modal__content__excerpt">${modalExcerpt}</div>`
                : '';
        let image =
            modalImageURL && modalImageURL !== 'undefined'
                ? `<picture class="modal__picture"><img class="modal__image" src="${modalImageURL}" alt="${modalImageAlt}" /></picture>`
                : '';
    ...
    document.body.addEventListener('click', (e) => {
        if (
            ...
            e.target.id === 'navigation-overlay'
        ) {
            closemodal();
        }
    });
